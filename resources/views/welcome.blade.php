        ...
        @section('content')
            <h2 style="text-align: center;">laravel Data Table:</h2>
            <h4>users:<h4>
               <p>@if(session()->has('message'))
                {{ session()->get('message')}}</p>
                @endif
                <h4>welcome {{auth()->user()->myName}}</h4>
                <a href="{{route('logoutpage')}}" class="btn btn-danger">LogOut</a>
                <a href="{{route('exportpage')}}" class="btn btn-success">Export</a>
                <a href="{{route('pdfpage')}}" class="btn btn-warning">PDF</a>
